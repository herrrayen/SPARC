export const sitePlaceholders = {
  congressTagline:
    "Out there is our home. It's time to fix it. Build sustainable technology inspired by the WALL-E mission.",
  congressDate: "April 11-12, 2026",
  congressLocation: "Tunisia",
  registrationStatus: "Specifications book released ahead of the build cycle",
  contactEmail: "",
  host: "IEEE ISTIC Student Branch and Chapters",
  eventName: "IEEE ISTIC Congress and Innovation Challenge 2026",
  eventTheme:
    "Directive Earth: A WALL-E Inspired Challenge for a Sustainable Future",
  whatIsIt:
    "A premier student-led tech congress and challenge focused on restoring and optimizing our planet's energy future through practical software, hardware, and data innovation.",
};

export const missionPhases = [
  {
    title: "Phase 1: EVE Technical Bootcamp and Development",
    period: "Pre-event",
    details:
      "After the challenge specifications release, participants receive intensive training in deep learning, IoT communication protocols, and robotics control logic, then spend weeks refining their solutions.",
  },
  {
    title: "Phase 2: Congress and Pitch Days",
    period: "April 11-12, 2026",
    details:
      "Two days of networking, expert workshops, and keynote sessions, culminating in jury pitching sessions and an awards ceremony for the strongest deployable prototypes.",
  },
];

export const innovationTracks = [
  {
    title: "Smart Home AI (The Axiom Alternative)",
    details:
      "Build algorithms that optimize household electricity usage and prove sustainable living can be intelligent and practical.",
  },
  {
    title: "EV Infrastructure (Data Science)",
    details:
      "Use predictive analytics to identify optimal electric vehicle charging station locations.",
  },
  {
    title: "Smart Mobility (IoT and Software)",
    details:
      "Develop a real-time app that guides drivers to the nearest available EV charger.",
  },
  {
    title: "The WALL-E Track (Hardware and Robotics)",
    details:
      "Prototype an autonomous robot that cleans solar panels, clears debris, and boosts renewable energy output.",
  },
];

export const aboutWidgets = [
  {
    title: "About SPARC",
    description:
      "SPARC is the event identity for a student-driven technology mission focused on energy resilience, sustainable innovation, and real-world deployment impact.",
    logo: "/assets/brand/sparc-logo-white.png",
    logoBg: "/assets/brand/sparc-bg.png",
    websiteLabel: "Visit SPARC Page",
    websiteUrl: "",
  },
  {
    title: "About POWER AND ENERGY SOCIETY",
    description:
      "IEEE PES is a global community advancing electric power and energy systems through research, standards, and professional collaboration.",
    logo: "/assets/collaborators/pes_logo.png",
    websiteLabel: "Visit IEEE PES",
    websiteUrl: "https://ieee-pes.org",
  },
  {
    title: "About IEEE ISTIC Student Branch",
    description:
      "The IEEE ISTIC Student Branch supports student leadership, technical upskilling, and project-based innovation across multiple engineering domains.",
    logo: "/assets/collaborators/istic-sb.png",
    websiteLabel: "Visit IEEE ISTIC Student Branch",
    websiteUrl: "https://istic.ieee.tn/",
  },
  {
    title: "About IEEE IAS PES ISTIC Student Branch Joint Chapter",
    description:
      "The joint chapter aligns industry applications and power-energy expertise to deliver impactful technical activities and collaborative student projects.",
    logo: "/assets/collaborators/ias-pes.png",
    websiteLabel: "Visit IEEE IAS PES ISTIC Student Branch Joint Chapter",
    websiteUrl: "https://ias-pes-istic.ieee.tn/",
  },
  {
    title: "About IEEE ISTIC Computer Society Student Branch Chapter",
    description:
      "The IEEE ISTIC Computer Society Student Branch Chapter empowers students through computing-focused activities in software engineering, systems, and emerging digital technologies.",
    logo: "/assets/collaborators/ieee_istic_color_noir.png",
    websiteLabel: "Visit IEEE ISTIC Computer Society Student Branch Chapter",
    websiteUrl: "https://www.facebook.com/profile.php?id=61552197270463",
  },
  {
    title: "About IEEE IES Student Branch Chapter",
    description:
      "The IEEE IES Student Branch Chapter promotes practical innovation in industrial electronics, automation, intelligent control, and real-world embedded systems.",
    logo: "/assets/collaborators/ies_istic.jpg",
    websiteLabel: "Visit IEEE IES Student Branch Chapter",
    websiteUrl: "https://www.facebook.com/profile.php?id=61587696301965",
  },
  {
    title: "About IEEE RAS ISTIC Student Branch Chapter",
    description:
      "The IEEE RAS ISTIC Student Branch Chapter develops robotics and automation talent through technical projects, workshops, and collaborative prototyping.",
    logo: "/assets/collaborators/ras.jpg",
    websiteLabel: "Visit IEEE RAS ISTIC Student Branch Chapter",
    websiteUrl: "https://www.facebook.com/profile.php?id=61555546985817",
  },
  {
    title: "About IEEE ISTIC SIGHT Group",
    description:
      "The IEEE ISTIC SIGHT Group supports humanitarian technology initiatives that address local community challenges through sustainable engineering impact.",
    logo: "/assets/collaborators/sight.jpg",
    websiteLabel: "Visit IEEE ISTIC SIGHT Group",
    websiteUrl: "https://www.facebook.com/profile.php?id=61553922900465",
  },
  {
    title: "About IEEE WIE ISTIC Affinity Group",
    description:
      "The IEEE WIE ISTIC Affinity Group advances inclusion and leadership by supporting women in engineering through mentorship, events, and technical growth opportunities.",
    logo: "/assets/collaborators/wie.jpg",
    websiteLabel: "Visit IEEE WIE ISTIC Affinity Group",
    websiteUrl: "https://www.facebook.com/ieeewie.istic",
  },
];

export const socialLinks = {
  facebook: "https://www.facebook.com/sparc.istic",
  instagram: "https://www.instagram.com/sparc_1.0/",
  linkedin: "https://www.linkedin.com/company/sparc-sustainable-power-renewable-advancement-congress/",
};

export const organizingCommittee = [
  {
    name: "Mohamed Rayen Trabelsi",
    role: "Event Chair",
    email: "trabelsimedrayen@ieee.org",
    linkedin: "https://www.linkedin.com/in/trabelsi-med-rayen/",
    photo: "/assets/oc/rayen.jpg",
  },
  {
    name: "Hamza Mellouli",
    role: "Event Vice Chair",
    email: "",
    linkedin: "",
    photo: "/assets/oc/hamza.jfif",
  },
  {
    name: "Kmar Ayari",
    role: "Event Secretary",
    email: "",
    linkedin: "",
    photo: "/assets/oc/kmar.webp",
  },
  {
    name: "Mira Ouertatani",
    role: "Event Treasurer",
    email: "miragc2006@gmail.com",
    linkedin: "",
    photo: "/assets/oc/mira.jpg",
  },
  {
    name: "Baha Eddine Gassar",
    role: "Social Media Lead",
    email: "bahagassar@ieee.org",
    linkedin: "https://www.linkedin.com/in/bahaeddinegassar",
    photo: "/assets/oc/baha.jpg",
  },
  {
    name: "Ranim Ben Mansour",
    role: "Media Manager",
    email: "ranimbenmansour1234@gmail.com",
    linkedin: "",
    photo: "/assets/oc/Ranim Benmansour.jpeg",
  },
  {
    name: "Sarah Soltani",
    role: "Ambassadors Coordinator",
    email: "soltanisarah98@gmail.com",
    linkedin: "https://www.linkedin.com/in/sarah-soltani-782450387?utm_source=share_via&utm_content=profile&utm_medium=member_ios",
    photo: "/assets/oc/sarah.jpeg",
  },
  {
    name: "Mouhib Farhat",
    role: "Technical Program Lead",
    email: "",
    linkedin: "",
    photo: "/assets/oc/mouhib.jpg",
  },
  {
    name: "Andel Belgacemi",
    role: "Logistics Lead",
    email: "andelguesmi@gmail.com",
    linkedin: "https://tn.linkedin.com/in/andel-belgacemi-40455b2a5",
    photo: "/assets/oc/andel-belgacemi.jpg",
  },
  {
    name: "Aziz Chaouch",
    role: "Transportation Manager",
    email: "",
    linkedin: "",
  },
  {
    name: "Khalil Derouiche",
    role: "Logistics Manager",
    email: "",
    linkedin: "",
  },
  {
    name: "Fatma Feki",
    role: "Industrial Visits Coordinator",
    email: "fatmafeki2006@gmail.com",
    linkedin: "http://www.linkedin.com/in/fatma-feki-444629360",
    photo: "/assets/oc/Fatma Feki.JPG",
  },
  {
    name: "Med Malek Krima",
    role: "Industrial Visits Coordinator",
    email: "krimamalek@gmail.com",
    linkedin: "",
    photo: "/assets/oc/malek.jpg",
  },
  {
    name: "Med Mounir Gharbi",
    role: "Technical Program Manager",
    email: "",
    linkedin: "",
  },
  {
    name: "Nagham Ben Hassan",
    role: "Technical Program Manager",
    email: "",
    linkedin: "",
  },
  {
    name: "Yosra Boughattas",
    role: "Sponsoring Manager",
    email: "",
    linkedin: "",
  },
  {
    name: "Salma Teyeb",
    role: "Participants Manager",
    email: "salma.teyeb.01@gmail.com",
    linkedin: "",
    photo: "/assets/oc/salma-teyeb.jpg",
  },
  {
    name: "Mariem Aouadi",
    role: "HR Manager",
    email: "",
    linkedin: "",
  },
  {
    name: "Yasmine Harzi",
    role: "Decoration Lead",
    email: "",
    linkedin: "",
  },
  {
    name: "Sinda Chekir",
    role: "Decoration Manager",
    email: "sinda.chekir1809@gmail.com",
    linkedin: "",
    photo: "/assets/oc/chekir sinda.jpg",
  },
  {
    name: "Tayssir Dardouri",
    role: "Decoration Manager",
    email: "tayssirdardouri5@gmail.com",
    linkedin: "",
    photo: "/assets/oc/Tayssir Dardouri.jpeg",
  },
  {
    name: "Omar Hamrouni",
    role: "Media Manager",
    email: "hamrouniomar95@gmail.com",
    linkedin: "https://www.linkedin.com/in/omar-hamrouni-858b733a5?utm_source=share_via&utm_content=profile&utm_medium=member_ios",
    photo: "/assets/oc/omar.jpeg",
  },
  {
    name: "Yahya Ben Mohamed",
    role: "Technical Program Manager",
    email: "yahyagrandfr@gmail.com",
    linkedin: "https://www.linkedin.com/in/yahya-ben-mohamed-2138aa392?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app",
    photo: "/assets/oc/yahya.jpg",
  },
  {
    name: "Mohamed Sehly",
    role: "Sponsoring Manager",
    email: "",
    linkedin: "",
  },
];

export const ieeePartners = [
  {
    name: "IEEE ISTIC Student Branch",
    logo: "/assets/collaborators/istic-sb.png",
    instagram: "https://www.instagram.com/ieee.istic.sb/",
    linkedin: "https://www.linkedin.com/company/ieee-istic-sb",
  },
  {
    name: "IEEE IAS PES ISTIC SB Joint Chapter",
    logo: "/assets/collaborators/ias-pes.png",
    instagram: "https://www.instagram.com/ieee_istic_ias_pes_sbjc/",
    linkedin: "https://www.linkedin.com/company/ieee-istic-ias-pes-student-branch-joint-chapter/",
  },
  {
    name: "IEEE PES",
    logo: "/assets/collaborators/pes_logo.png",
    instagram: "https://www.instagram.com/ieee.pes/",
    linkedin: "https://www.linkedin.com/company/ieeepes",
  },
  {
    name: "IEEE IAS",
    logo: "",
    instagram: "",
    linkedin: "",
  },
];

export const collaborators = [
  {
    name: "Nova Scale",
    role: "Video Production Agency",
    logo: "/assets/collaborators/nova-scale.png",
    instagram: "https://www.instagram.com/novascale.media/",
    linkedin: "",
  },
];

export const sponsorPlaceholders = [];

export const ambassadorProfiles = [
  {
    order: 1,
    name: "Abdus Sobhan",
    studentBranch: "",
    region: "International",
    photo: "/assets/ambassadors/international/Abdus Sobhan - Abdus Sobhan.jpg",
    logo: "/assets/ambassadors/logos/0TL50Y4P - Abdus Sobhan.jpg",
  },
  {
    order: 2,
    name: "Ayelagbe Timothy",
    studentBranch: "",
    region: "International",
    photo: "/assets/ambassadors/international/Headshot WFF - Ayelagbe Timothy.jpg",
    logo: "/assets/ambassadors/logos/IEEE OAUSB LOGOO - Ayelagbe Timothy.png",
  },
  {
    order: 3,
    name: "Nadia Blaibleh",
    studentBranch: "",
    region: "International",
    photo: "/assets/ambassadors/international/pic - Nadia Blaibleh.jpeg",
    logo: "/assets/ambassadors/logos/logo - Nadia Blaibleh.png",
  },
  {
    order: 4,
    name: "Cagdas Ozan Pamuk",
    studentBranch: "",
    region: "International",
    photo: "/assets/ambassadors/international/Profile Pic (Large) - Cagdas Ozan Pamuk.png",
    logo: "/assets/ambassadors/logos/IEEE Türkiye Section - Cagdas Ozan Pamuk.jpg",
  },
  {
    order: 5,
    name: "Douae Zouaoui",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/1758651910074 - Douae Zouaoui.png",
    logo: "/assets/ambassadors/logos/1 (1) - Douae Zouaoui.png",
  },
  {
    order: 6,
    name: "Kouki eya",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/418834236_397756572962296_4869241113253091289_n - Kouki eya.jpg",
    logo: "/assets/ambassadors/logos/SB Color - Kouki eya.png",
  },
  {
    order: 7,
    name: "Ranim Mahdi",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/641546722_2050936055828697_9093850205938034802_n - Ranim Mahdi.jpg",
    logo: "/assets/ambassadors/logos/fsb.jpg",
  },
  {
    order: 8,
    name: "Amal Lenda",
    studentBranch: "ISIMS SB",
    region: "Local",
    photo: "/assets/ambassadors/local/amal amb - Amal Lenda.jpg",
    logo: "/assets/ambassadors/logos/isims.jpg",
  },
  {
    order: 9,
    name: "Nouha Trabelsi",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/ca892d6f-ff1d-4a48-bbde-2649549f374e-removebg-preview - Nouha Trabelsi.png",
    logo: "/assets/ambassadors/logos/622970429_1471327405001700_3165341562334085728_n - Nouha Trabelsi.jpg",
  },
  {
    order: 10,
    name: "Dridi Nour",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/IMG-20251018-WA0000 - Dridi Nour.jpg",
    logo: "/assets/ambassadors/logos/blue - Dridi Nour.png",
  },
  {
    order: 11,
    name: "CHABANI NOUR HOUDA",
    studentBranch: "FSB",
    region: "Local",
    photo: "/assets/ambassadors/local/IMG-20251108-WA0364 - CHBANI NOUR ELHOUDA.jpg",
    logo: "/assets/ambassadors/logos/fsb.jpg",
  },
  {
    order: 12,
    name: "Houssem Boughizane",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/IMG-20260128-WA0001 - Houssem Boughizane.jpg",
    logo: "/assets/ambassadors/logos/Screenshot_20260319_154628_Gallery - Houssem Boughizane.jpg",
  },
  {
    order: 13,
    name: "louaa louati",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/IMG_3155 - louaa louati.JPG",
    logo: "/assets/ambassadors/logos/logo black sb - louaa louati.png",
  },
  {
    order: 14,
    name: "Mohamed Saidi",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/mohamed_saidi - Mohamed Saidi.png",
    logo: "/assets/ambassadors/logos/IMG_6631 (1) - Mohamed Saidi.PNG",
  },
  {
    order: 15,
    name: "Tesnim Maâtalli",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/photo  - Tesnim Maâtalli.png",
    logo: "/assets/ambassadors/logos/ieee essths - Tesnim Maâtalli.png",
  },
  {
    order: 16,
    name: "Ben Hoiaia Amine",
    studentBranch: "",
    region: "Local",
    photo: "/assets/ambassadors/local/photo - Ben Hoiaia Amine.png",
    logo: "/assets/ambassadors/logos/Ieee issatm sb blue (1) - Ben Hoiaia Amine.png",
  },
];

export const venueGalleryImages = [
  "/assets/brand/ccsj-ben-arous-1.jpg",
  "/assets/brand/ccsj-ben-arous-2.jpg",
  "/assets/brand/ccsj-ben-arous-3.jpg",
  "/assets/brand/ccsj-ben-arous-4.jpg",
];

export const speakerProfiles = [
  {
    name: "Lina Bertling Tjernberg",
    role: "Keynote Speaker",
    organization: "KTH Royal Institute of Technology",
    email: "linab@kth.se",
    linkedin: "https://www.linkedin.com/in/linabertling/",
    photo: "/assets/speakers/Lina-Bertling-Tjernberg.jpg",
  },
  {
    name: "Manef Bourogaoui",
    role: "Speaker",
    organization: "Organization to be announced",
    email: "manef.bourogaoui@enit.utm.tn",
    linkedin: "",
    photo: "/assets/speakers/manef.jpg",
  },
  {
    name: "Ayoub Madyouni",
    role: "Speaker",
    organization: "Organization to be announced",
    email: "ayoub_madyouni@ieee.org",
    linkedin: "",
    photo: "/assets/speakers/ayoub.jpg",
  },
  {
    name: "Hamza Mellouli",
    role: "Speaker",
    organization: "Organization to be announced",
    email: "hamzamellouli@ieee.org",
    linkedin: "",
    photo: "/assets/speakers/hamza.jfif",
  },
  {
    name: "Molka Weslati",
    role: "Speaker",
    organization: "Organization to be announced",
    email: "molkaweslati@ieee.org",
    linkedin: "",
    photo: "/assets/speakers/Molka Weslati (2).jpg",
  },
  {
    name: "Ibtihel Trabelsi",
    role: "Speaker",
    organization: "Organization to be announced",
    email: "ibtihel.trabelsi26@ieee.org",
    linkedin: "",
    photo: "/assets/speakers/ibtihel.jpg",
  },
];
